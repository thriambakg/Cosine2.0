...
module "api_gateway" {
  ...
  # Updated to include presigned URL endpoints
  # Note: New nested resources (files/presigned, files/complete) are created outside the module
  # and will be included in the deployment when this trigger is incremented
  deployment_trigger = "86" # Added presigned URL file upload endpoints
}

# Nested resources for presigned URL file uploads (files/presigned and files/complete)
# These are nested under the "files" resource created by the module
resource "aws_api_gateway_resource" "files_presigned" {
...
resource "aws_api_gateway_resource" "files_complete" {
  rest_api_id = module.api_gateway.rest_api_id
  parent_id   = module.api_gateway.resource_ids["files"]
  path_part   = "complete"
...
# Methods for presigned URL endpoints
resource "aws_api_gateway_method" "files_presigned_post" {
  rest_api_id   = module.api_gateway.rest_api_id
  resource_id   = aws_api_gateway_resource.files_presigned.id
  http_method   = "POST"
...
resource "aws_api_gateway_method" "files_complete_post" {
  rest_api_id   = module.api_gateway.rest_api_id
  resource_id   = aws_api_gateway_resource.files_complete.id
  http_method   = "POST"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = module.api_gateway.authorizer_id
...
# Integrations for presigned URL endpoints
resource "aws_api_gateway_integration" "files_presigned_post" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_presigned.id
  http_method = aws_api_gateway_method.files_presigned_post.http_method
...
resource "aws_api_gateway_integration" "files_complete_post" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_complete.id
  http_method = aws_api_gateway_method.files_complete_post.http_method
...
# Method responses for presigned URL endpoints
resource "aws_api_gateway_method_response" "files_presigned_post" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_presigned.id
  http_method = aws_api_gateway_method.files_presigned_post.http_method
  status_code = "200"
...
resource "aws_api_gateway_method_response" "files_complete_post" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_complete.id
  http_method = aws_api_gateway_method.files_complete_post.http_method
  status_code = "200"
...
# Integration responses for presigned URL endpoints
resource "aws_api_gateway_integration_response" "files_presigned_post" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_presigned.id
  http_method = aws_api_gateway_method.files_presigned_post.http_method
  status_code = aws_api_gateway_method_response.files_presigned_post.status_code

  response_parameters = {
    "method.response.header.Access-Control-Allow-Origin" = "'*'"
  }

  depends_on = [aws_api_gateway_integration.files_presigned_post]
...
resource "aws_api_gateway_integration_response" "files_complete_post" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_complete.id
  http_method = aws_api_gateway_method.files_complete_post.http_method
  status_code = aws_api_gateway_method_response.files_complete_post.status_code

  response_parameters = {
    "method.response.header.Access-Control-Allow-Origin" = "'*'"
  }

  depends_on = [aws_api_gateway_integration.files_complete_post]
...
resource "aws_api_gateway_method" "files_presigned_options" {
  rest_api_id   = module.api_gateway.rest_api_id
  resource_id   = aws_api_gateway_resource.files_presigned.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_method" "files_complete_options" {
  rest_api_id   = module.api_gateway.rest_api_id
  resource_id   = aws_api_gateway_resource.files_complete.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
# OPTIONS integrations (mock)
resource "aws_api_gateway_integration" "files_presigned_options" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_presigned.id
  http_method = aws_api_gateway_method.files_presigned_options.http_method
...
resource "aws_api_gateway_integration" "files_complete_options" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_complete.id
  http_method = aws_api_gateway_method.files_complete_options.http_method
...
resource "aws_api_gateway_method_response" "files_presigned_options" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_presigned.id
  http_method = aws_api_gateway_method.files_presigned_options.http_method
  status_code = "200"
  ...
  depends_on = [aws_api_gateway_method.files_presigned_options]
...
resource "aws_api_gateway_method_response" "files_complete_options" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_complete.id
  http_method = aws_api_gateway_method.files_complete_options.http_method
  status_code = "200"
  ...
  depends_on = [aws_api_gateway_method.files_complete_options]
...
resource "aws_api_gateway_integration_response" "files_presigned_options" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_presigned.id
  http_method = aws_api_gateway_method.files_presigned_options.http_method
  status_code = aws_api_gateway_method_response.files_presigned_options.status_code
  ...
  depends_on = [
    aws_api_gateway_integration.files_presigned_options,
  ]
...
resource "aws_api_gateway_integration_response" "files_complete_options" {
  rest_api_id = module.api_gateway.rest_api_id
  resource_id = aws_api_gateway_resource.files_complete.id
  http_method = aws_api_gateway_method.files_complete_options.http_method
  status_code = aws_api_gateway_method_response.files_complete_options.status_code
  ...
  depends_on = [
    aws_api_gateway_integration.files_complete_options,
  ]
...
resource "aws_lambda_permission" "files_presigned_post" {
  ...
  source_arn    = "${module.api_gateway.execution_arn}/*/POST/files/presigned"
}

resource "aws_lambda_permission" "files_complete_post" {
  statement_id  = "AllowExecutionFromAPIGateway_files_complete_post"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.chat_agent.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${module.api_gateway.execution_arn}/*/POST/files/complete"
}
