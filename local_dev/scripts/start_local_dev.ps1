...
if (-not (Test-Path "venv\Scripts\python.exe")) {
    Write-Host "Virtual environment not found. Please run local_dev\scripts\setup_full_local.ps1 first." -ForegroundColor Red
    exit 1
}

# Check if frontend dependencies are ready
if (-not (Test-Path "frontend\app\node_modules")) {
    Write-Host "Frontend dependencies not found. Please run local_dev\scripts\setup_full_local.ps1 first." -ForegroundColor Red
    exit 1
}

Write-Host "`nStarting backend server..." -ForegroundColor Yellow
# Start backend in background
$backend = Start-Process -FilePath ".\venv\Scripts\python.exe" -ArgumentList "local_dev\backend\local_server.py" -PassThru -WindowStyle Normal
