# Cosine AI Chat - Enhanced Setup & Start Script
...
foreach ($package in $packages) {
    try {
        $result = & $pythonCommand -c "import $package; print('OK')" 2>$null
        if ($result -eq "OK") {
...
try {
    ...
    Write-Host "  1. Make sure you are in the local_dev directory" -ForegroundColor White
    Write-Host "  2. Check that chat_server.py exists in this directory" -ForegroundColor White
    Write-Host "  3. Try running: $pythonCommand chat_server.py directly" -ForegroundColor White
