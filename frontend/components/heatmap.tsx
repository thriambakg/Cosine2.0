...
export function Heatmap() {
  ...
  const generateHeatmaps = async () => {
    ...
    const mockHeatmapData: HeatmapData = {
      call: {
        z: Array(20).fill(0).map(() => Array(20).fill(0).map(() => Math.random() * 50)),
        x: Array(20).fill(0).map((_, i) => parseFloat(minS) + (parseFloat(maxS) - parseFloat(minS)) * i / 19),
        y: Array(20).fill(0).map((_, i) => parseFloat(minSigma) + (parseFloat(maxSigma) - parseFloat(minSigma)) * i / 19),
      },
      put: {
        z: Array(20).fill(0).map(() => Array(20).fill(0).map(() => Math.random() * 50)),
        x: Array(20).fill(0).map((_, i) => parseFloat(minS) + (parseFloat(maxS) - parseFloat(minS)) * i / 19),
