...
- search_congress_bills(filters, limit=5, last_evaluated_key, roll_call_search, question_date, roll_call_details) - Search congressional bills, roll call lists, OR fetch full details for one roll call
  * **Roll call details (single roll)**: When user has a roll call in context or asks for full vote details for one roll, use roll_call_details (JSON). **Always include search_index_sk from the context item when present** for exact PK/SK lookup (e.g. roll_call_details='{"search_index_sk": "119#2026-01-15#1#40"}'); otherwise use congress, session, roll.
  * **Bill search**: Use filters (sponsor_name, policy_area, etc.). Use search_autocomplete for sponsor_name and policy_area.
  * **Roll call search (list)**: Use roll_call_search (JSON). When searching by politician: (1) call search_autocomplete(politician_name, "congress_legislator") to get matches; (2) use the **politician_id (bioguide_id)** from each match (e.g. "C000127") in roll_call_search.politician_ids—roll call search requires bioguide_id; other bill filters (e.g. sponsor_name) may use names. Default congress via question_date when needed.
  * SEARCH#ROLL list: roll_call_search='{"search_index": "SEARCH#ROLL", "congress": 119, "session": 1, "limit": 20}' or omit congress and set question_date.
  ...
  * Returns: roll_call_details: { result: { roll_item (includes vote_question, result, result_display, vote_type, latest_action_date), bill_associated, vote_summary, members } }; bill search: { results, count, has_more, last_evaluated_key: { offset, total_items } }; roll call search: { results, enriched_results, count, has_more } — use enriched_results for display (bill_title, roll_date, vote_type, vote_question, result_display, congress, session, roll)
