...
class ContextAwareAgent:
    ...
    def __init__(self):
        ...
        get_logger().info("ContextAwareAgent system initialized")
    ...
    def get_session_agent(self, session_context: Dict[str, Any], model_name: str = 'claude-sonnet-4') -> Any:
        ...
        try:
            ...
            get_logger().info(f"🔍 DEBUG: Getting session agent for session {session_id} with model {model_name}")
            get_logger().info(f"🔍 DEBUG: Agent key: {agent_key}")
            get_logger().info(f"🔍 DEBUG: Current cached agents: {list(self.session_agents.keys())}")
            
            # Check if we already have a cached agent for this session and model
            if agent_key in self.session_agents:
                get_logger().info(f"🔍 DEBUG: Using cached agent for session {session_id} with model {model_name}")
            ...
            get_logger().info(f"🔍 DEBUG: Existing agent keys for session {session_id}: {existing_agent_keys}")
            
            if existing_agent_keys and not any(key.endswith(f"_{model_name}") for key in existing_agent_keys):
                get_logger().info(f"🔍 DEBUG: Model switch detected for session {session_id}, clearing old agent cache")
                get_logger().info(f"🔍 DEBUG: Switching from {existing_agent_keys} to {model_name}")
                # Clear old agents for this session to ensure fresh context
                for old_key in existing_agent_keys:
                    get_logger().info(f"🔍 DEBUG: Clearing old agent: {old_key}")
            ...
            get_logger().info(f"Created new context-aware agent for session {session_id} with model {model_name}")
            return agent
            
        except Exception as e:
            get_logger().error(f"Error creating session agent: {str(e)}")
    ...
    def _create_session_agent(self, session_context: Dict[str, Any], model_name: str = 'claude-sonnet-4') -> Any:
        ...
        try:
            ...
            if model_name not in MODELS:
                get_logger().warning(f"Unknown model '{model_name}', falling back to claude-sonnet-4")
                model_name = 'claude-sonnet-4'
            
            selected_model = MODELS[model_name]
            get_logger().info(f"Creating session agent with model: {model_name}")
    ...
    def _add_conversation_history_to_prompt(self, system_prompt: str, session_context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not conversation_history:
                get_logger().info("🔍 DEBUG: No conversation history to add to system prompt")
                return system_prompt
            
            get_logger().info(f"🔍 DEBUG: Adding {len(conversation_history)} conversations to system prompt for model switching")
            ...
            get_logger().info(f"🔍 DEBUG: Added conversation history to system prompt: {len(history_section)} characters")
            get_logger().info(f"🔍 DEBUG: History section content: {history_section[:200]}...")
            
            return system_prompt + history_section
            
        except Exception as e:
            get_logger().error(f"🔍 DEBUG: Error adding conversation history to prompt: {str(e)}")
...
"""
            ...
            get_logger().error(f"Error formatting session context: {str(e)}")
    ...
    def _get_session_tools(self, session_context: Dict[str, Any]) -> List:
        ...
        try:
            ...
            if page_type == 'crypto':
                ...
                get_logger().info("Adding crypto-specific tools to session")
            elif page_type == 'portfolio':
                # Add portfolio-specific tools if available
                get_logger().info("Adding portfolio-specific tools to session")
            elif page_type == 'stocks':
                # Add stock-specific tools if available
                get_logger().info("Adding stock-specific tools to session")
            
            return session_tools
            
        except Exception as e:
            get_logger().error(f"Error getting session tools: {str(e)}")
    ...
    def clear_session_cache(self, session_id: str) -> None:
        ...
        if session_id in self.session_agents:
            ...
            get_logger().info(f"Cleared cached agent for session {session_id}")
    ...
    def get_session_summary(self, session_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            get_logger().error(f"Error getting session summary: {str(e)}")
            return {'error': str(e)}
