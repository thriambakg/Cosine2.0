...
- search_congress_bills(filters, limit=5, last_evaluated_key, roll_call_search, question_date, roll_call_details) - Search congressional bills, roll call lists, OR fetch full details for one roll call
  ...
  * Returns: roll_call_details: { result: { roll_item, bill_associated, vote_summary, members } }; bill search: { results, count, has_more, last_evaluated_key: { offset, total_items } }; roll call search: { results, enriched_results, count, has_more } — use enriched_results for display (bill_title, roll_date, vote_type, congress, session, roll)
